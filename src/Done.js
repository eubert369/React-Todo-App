...
function DoneTodo(props) {
  ...
  var fetchDone = async () => {
    setLoading("Loading ...");
    await getDocs(collection(db, "finished")).then((query) => {
          ...
          {getDone.map((item, i) => {
            if (
              ...
              return (
                <div
                  key={i}
                  className="border justify-content-start mx-5 mx-auto px-4 py-3 rounded-4 border-dark bg-light mb-3 position-relative"
                >
                  <p className="my-0 text-start text-success fw-bold">
                    Status: (Done)
                  </p>
                  ...
                  <p className="my-0 text-start fw-bold">
                    Date Finished: {item.date_finished}
                  </p>
                  ...
                  <div className="position-absolute top-0 end-0 d-flex mx-3 mt-2">
                    <button
                      className="btn btn-danger rounded-5 ms-1 fw-bold action-btn"
                      onClick={() => {
